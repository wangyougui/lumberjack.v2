...
func compile(handler compileHandler, p string, ds SpecificationSet) error {
	for l := len(p); l > 0; l = len(p) {
		...
		if err != nil {
			return errors.Join(err, fmt.Errorf(`pattern compilation failed`))
		}
...
func Format(p string, t time.Time, options ...Option) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get specification set")
	}
	...
	if err := compile(h, p, ds); err != nil {
		return "", fmt.Errorf("failed to compile format")
	}
...
// NewStf creates a new Strftime object. If the compilation fails, then
// an error is returned in the second argument.
func NewStf(p string, options ...Option) (*Strftime, error) {
	// TODO: this may be premature optimization
	ds, err := getSpecificationSetFor(options...)
	if err != nil {
		return nil, fmt.Errorf("failed to get specification set")
	}
	...
	if err := compile(&h, p, ds); err != nil {
		return nil, fmt.Errorf("failed to compile format")
	}
